...
class SaddlePointProblem(cp.Problem):
    ...
    def solve(self, eps: float = 1e-1, *args, **kwargs: dict) -> float:  # noqa
        """
        ...
        assert np.isclose(np.divide(self.x_prob.value, -self.y_prob.value),1, atol=eps) or np.isclose(
            -self.y_prob.value, self.x_prob.value, atol=eps
        ), f"Difference between x and y problem is {diff} for tolerance {eps}, (should be 0)."
